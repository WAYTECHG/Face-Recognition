...
class Improved_PCA_FaceRecognizer(Preprocessing):
    def __init__(self):
        ...
        #default
        self.data_preparation()
        self.best_components = 25
        self.best_components_lda = 10
        self.best_thres = 1.1
        self.max_acc = 0
    ...
    def display_every_component_analysis(self):
        component_acc_tuple = []
        plot_data = []
        maximum_label = len(np.unique(self.y))
        for i in range(1, maximum_label-1, 1):

            pca = PCA(whiten=True, n_components=self.best_components)
            self.x_train_new = pca.fit_transform(self.x_train)
            self.x_test_new = pca.transform(self.x_test)

            """New Features"""
            lda = LDA(n_components = i)
            self.x_train_new = lda.fit_transform(self.x_train_new,self.y_train)
            self.x_test_new = lda.transform(self.x_test_new)
            """New Features"""
            ...
            ran_arr = np.append(np.random.rand(30) * 3, np.random.rand(20) * 2)
            ...
            print("Number of Components (", i, ") The best threshold is", best_thres, "and the maximum accuracy is", max_acc)
        ...
        fig1.savefig('plots_1_fisherface.png')
        ...
        plt.savefig('FisherFace_Relationship_between_components_accuracy.png')
        plt.show()
        ...
        self.best_components_lda = int(input("Please input the best number of lda components: "))
        self.best_thres = float(input("Please input the best distance threshold: "))

    def train_eigenface(self):
        # Apply PCA on th e data
        # print("The default number of components is 15")
        # self.best_components = int(input("Please enter the number of components to train: "))
        ...
        """New Features"""
        lda = LDA(n_components=self.best_components_lda)
        self.x_train_new = lda.fit_transform(self.x_train_new, self.y_train)
        self.x_test_new = lda.transform(self.x_test_new)

        print("The Shape of x train data after LDA is", self.x_train_new.shape)
        print("The Shape of x test data after LDA is", self.x_test_new.shape)
        """New Features"""
        ...
        #Save PCA model
        joblib.dump(pca, 'pca_x_train_fisher.joblib')
        #Save LDA model
    ...
    def load_pca_model(self):
        pca = joblib.load("pca_x_train_fisher.joblib")
        print("pca loaded successfully.")
        return pca

    def load_lda_model(self):
        lda = joblib.load("lda.joblib")
        print("lda loaded successfully.")
        return lda
    ...
    def recognize_faces(self):
        ...
        while True:
            ...
            for (x, y, w, h) in faces:
                ...
                flattened_face = LDA.transform(flattened_face)
...
if __name__ == "__main__":
    ...
    while True:
        print("-"*80)
        print("\t\tFISHERFACE RECOGNITION SYSTEM")
        print("-"*80)
        print("1. Capture Training Image")
        print("2. Display Every Component")
        print("3. Train FisherFace Recognition System")
